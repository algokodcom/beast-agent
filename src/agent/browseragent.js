  ...
  for (let step = 1; step <= maxSteps; step++) {
    ...
    try {
      if (kind === 'fill') {
        ...
        result = await deps.act('type', { ref: Number(action.ref), text });
        await deps.wait({ ms: FILL_SETTLE_MS });
      } else if (kind === 'click') {
        result = await deps.act('click', { ref: Number(action.ref) });
        await deps.wait({ ms: 120 });
      } else if (kind === 'select') {
        const value = action.value != null && String(action.value) !== '' ? action.value : baseLabel(action.label);
        result = await deps.act('select', { ref: Number(action.ref), value: String(value) });
        await deps.wait({ ms: 120 });
      } else if (kind === 'scroll') {
        result = await deps.act('scroll', { direction: Number(action.delta) < 0 ? 'up' : 'down' });
        await deps.wait({ ms: 120 });
      } else if (kind === 'wait') {
    ...
    if (result && result.changed === false && kind !== 'wait' && kind !== 'scroll') {
      noChangeStreak++;
