'use strict';
// MT5 DURUM ENVANTERİ — hesap + pozisyonlar + bekleyen emirler (tek köprü oturumunda)
...
(async function main() {
  const r = await call([
    ...
    { method: 'positions', params: {} },
    { method: 'orders', params: {} }
  ]);
  if (!r.ok) return console.log(JSON.stringify({ ok: false, error: r.error || 'durum alinamadi' }));
  const out = { ok: true };
  if (r.results && r.results[0] && r.results[0].ok) out.account = r.results[0].data.account;
  else out.account_hata = (r.results && r.results[0] && r.results[0].error) || '?';
  if (r.results && r.results[1] && r.results[1].ok) out.positions = r.results[1].data.positions;
  else out.positions_hata = (r.results && r.results[1] && r.results[1].error) || '?';
  if (r.results && r.results[2] && r.results[2].ok) out.orders = r.results[2].data.orders;
  else out.orders_hata = (r.results && r.results[2] && r.results[2].error) || '?';
