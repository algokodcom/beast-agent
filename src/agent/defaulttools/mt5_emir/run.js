'use strict';
// MT5 PİYASA EMİRİ AÇ — stdin: JSON args · stdout: JSON sonuç
  ...
  return new Promise((resolve) => {
    ...
    py.stdout.on('data', (d) => {
      ...
      while ((i = buf.indexOf('\n')) >= 0) {
        ...
        if (o && o.event) continue; // bridge olay satiri (baglanti raporu)
...
(async function main() {
  ...
  if (!symbol) return console.log(JSON.stringify({ ok: false, error: 'symbol zorunlu (örn XAUUSD)' }));
  if (side !== 'buy' && side !== 'sell') return console.log(JSON.stringify({ ok: false, error: 'side: buy | sell' }));
  if (!(volume > 0)) return console.log(JSON.stringify({ ok: false, error: 'volume (lot) zorunlu ve 0 dan buyuk olmali' }));
  const params = { symbol: symbol, side: side, volume: volume };
  if (args.sl) params.sl = Number(args.sl);
  if (args.tp) params.tp = Number(args.tp);
  if (args.comment) params.comment = String(args.comment);
  const r = await call([{ method: 'market', params: params }]);
  if (r.ok && r.data && r.data.result) {
    const res = r.data.result;
    console.log(JSON.stringify({ ok: true, ticket: res.order, retcode: res.retcode, comment: res.comment, fiyat: r.data.price, detay: r.data }));
  } else {
    console.log(JSON.stringify({ ok: false, error: (r.error || 'emir reddedildi') }));
  }
})();
