'use strict';
// MT5 CANLI FİYAT — stdin: JSON args · stdout: JSON sonuç
...
(async function main() {
  ...
  let syms = args.symbols;
  if (typeof syms === 'string') syms = [syms];
  if (!Array.isArray(syms) || syms.length === 0) return console.log(JSON.stringify({ ok: false, error: 'symbols dizisi zorunlu, orn [\"XAUUSD\"]' }));
  const r = await call([{ method: 'symbols', params: { symbols: syms } }]);
  if (r.ok && r.data) return console.log(JSON.stringify({ ok: true, symbols: r.data.symbols }));
  console.log(JSON.stringify({ ok: false, error: r.error || 'fiyat alinamadi' }));
})();
