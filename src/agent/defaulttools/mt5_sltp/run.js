'use strict';
// MT5 SL/TP GUNCELLE — stdin: JSON args · stdout: JSON sonuç
...
(async function main() {
  ...
  if (args.sl === undefined && args.tp === undefined) return console.log(JSON.stringify({ ok: false, error: 'sl veya tp vermelisin' }));
  const params = { ticket: ticket, sl: Number(args.sl || 0), tp: Number(args.tp || 0) };
  const r = await call([{ method: 'modify', params: params }]);
  if (r.ok && r.data && r.data.result) {
    const res = r.data.result;
    console.log(JSON.stringify({ ok: true, retcode: res.retcode, comment: res.comment, detay: r.data }));
  } else {
    console.log(JSON.stringify({ ok: false, error: r.error || 'SL/TP guncelleme reddedildi' }));
  }
})();
