...
(function () {
  const DICT = {
    tr: {
      ...
      tipAgentDm: 'Ajan DM — ajanlar arası konuşma',
  ...
  sess_del_confirm: 'Bu sohbet oturumunu silmek istediğine emin misin?',
      ...
      p_custom_sub: 'API adres + key ver, modeller otomatik çekilir; olmayanları elle ekle',
      p_custom_del: 'Sil',
      p_name_opt: 'Ad (opsiyonel)',
      p_api_url: 'API adresi',
      p_api_key: 'API Key',
      p_fetch: 'Modelleri Çek',
      p_models_manual: 'Ek modeller — elle ekle (her satıra bir ad; çekilenlere eklenir)',
      p_save: 'Kaydet',
      p_model_toast: 'Model: ',
      p_del_toast: 'Silindi: ',
      ...
      stt_engine: 'STT motoru',
      stt_seg_auto: 'Otomatik',
      stt_seg_whisper: 'Whisper (yerel)',
      tts_stt_title: 'Ses → Yazı',
      tts_stt_sub: 'Mikrofon kaydını yazıya çeviren motor',
      tts_tts_title: 'Yazı → Ses',
      tts_tts_sub: 'Sesli yanıt motoru ve ses seçimi',
      stt_engine_auto: 'Otomatik — anahtar varsa bulut, yoksa yerel Whisper',
      stt_engine_local: 'Whisper — yerel/offline (large-v3-turbo)',
      stt_engine_groq: 'Groq Whisper — bulut (hızlı)',
      stt_engine_openai: 'OpenAI-uyumlu Whisper — bulut',
      tts_note: 'Edge TTS: ücretsiz bulut motoru — anahtar gerekmez (Ahmet/Emel vb.). Piper: tamamen yerel/offline neural motor (ilk kullanımda ~80MB indirir). OpenAI motoru: uyumlu speech API (tts-1, tts-1-hd; sesler alloy, echo, fable, onyx, nova, shimmer).',
      tts_engine: 'Motor',
      tts_edge_voice: 'Edge sesi',
      tts_piper_voice: 'Piper sesi',
      tts_piper_download: 'Piper\'ı indir / hazırla',
      tts_piper_speed: 'Konuşma hızı',
      tts_piper_silence: 'Cümle sonu duraklama',
      tts_piper_noise: 'İfade (doğallık)',
      ...
      ws_h2: 'Web Arama',
      ws_sub: 'Arama zinciri: 1) SearXNG (yerel — çalışıyorsa otomatik öne alınır) 2) stealth (Chrome TLS taklidi) 3) dahili tarayıcı (direk Google) 4) TinyFish (anahtar varsa) 5) python çoklu-motor (ddgs / DDG+Bing+Mojeek). Sıra ve aç/kapa aşağıdan değiştirilir.',
      so_h2: 'Arama Sırası',
      ...
      tip_bb_restart: 'Yeniden başlat — takılan sayfayı sıfırdan açar (oturum korunur)',
      ...
      st_open: 'Aç',
      st_del: 'Sil',
      sec_toggle: 'İzin iste: yalnızca silme işlemlerinde onay sor (Beast Code, Sandbox dahil)',
      sec_sub: 'Kapalıyken (varsayılan) HİÇBİR yerde izin istenmez — Beast Code, Sandbox ve tüm araçlar serbest; tüm yetki ajandadır. Açıkken YALNIZCA SİLME işlemlerinde onay istenir: sohbette onay kartı + WhatsApp\u2019a mesaj gider. Diğer işlemler (yazma, düzenleme, komut çalıştırma, mail…) sorulmaz.',
      ...
      sec_on_toast: 'Onay açık — yalnızca silme işlemlerinde ajan sorar',
      sec_off_toast: 'Onay kapalı — hiçbir yerde sorulmaz, her şey serbest',
      sec_ok: 'Onayla',
      ...
      w_pane: 'İZLEYİCİLER',
      w_empty: 'Kayıtlı izleyici yok.',
      tipWatchLog: 'İzleyici loglarını gör',
      w_log_title: 'İzleyici Logları',
      w_log_clear: 'Seçili izleyicinin loglarını temizle',
      w_log_cleared: 'Loglar temizlendi',
      w_log_empty: 'Bu izleyici için henüz log kaydı yok.',
      w_log_ev_ok: 'kontrol',
      w_log_ev_trigger: 'tetik',
      w_log_ev_error: 'hata',
      ...
      tipSandbox: 'Beast Sandbox — GitHub reposu indir & içinde çalış',
      tipMcp: 'Beast MCP — dış araç sunucuları (Model Context Protocol)',
      ...
      store_likes: 'beğeni',
      store_builtin: 'Yerleşik',
      store_builtin_note: 'Ajanla birlikte gelir',
      ...
      /* Cron paneli + WhatsApp kişi/yetki editörü */
      ...
      /* Cron düzenleme */
      cr_update: 'Güncelle',
      cr_cancel_edit: 'Vazgeç',
      cr_edit_hint: 'tıkla: düzenle',
      cr_edit_loaded: 'Görev forma yüklendi — düzenleyip Güncelle',
      cr_updated: 'Görev güncellendi',
      cr_added: 'Görev eklendi',
      cr_edit_cancelled: 'Düzenleme iptal edildi',
      /* İzleyici ekleme formu */
      w_add: '+',
      w_added: 'İzleyici eklendi',
      w_add_name: 'İsim',
      w_add_url: 'https://...',
      w_add_path: 'json path (örn: data.price)',
      w_add_value: 'eşik değer',
      w_add_re: 'regex (opsiyonel)',
      w_kind_web: 'Web',
      w_kind_battery: 'Pil',
      w_kind_logs: 'Log',
      w_op_changed: 'değişti',
      /* Ajanda */
      aj_h2: 'Ajanda',
      aj_sub: 'Yaklaşan cron görevleri, hatırlatıcılar ve izleyiciler — en yakın üstte.',
      aj_empty: 'Planlanmış bir şey yok.',
      aj_now: 'şimdi',
      aj_later: 'sonra',
      aj_ago: 'önce',
      aj_notify: 'Masaüstü bildirimleri (toast) — cron, hatırlatıcı, izleyici ve ajan bitişinde',
      aj_notify_on: 'Bildirimler açık',
      aj_notify_off: 'Bildirimler kapalı',
      /* E-posta paneli */
      mail_inbox: 'Gelen Kutusu',
      mail_refresh: 'Yenile',
      mail_compose: 'Yeni Mail',
      mail_to: 'alıcı@ornek.com',
      mail_subject: 'Konu',
      mail_body: 'Mesaj…',
      mail_send: 'Gönder',
      mail_sent: 'Mail gönderildi',
      mail_send_fail: 'Mail gönderilemedi',
      mail_empty: 'Gelen kutusu boş.',
      mail_reply: 'Yanıtla',
      mail_read_fail: 'Mail okunamadı',
      ...
      ts_status_set: 'TypeSafe aktif — kayıtlı anahtar: *** · model: ',
      ts_status_unset: 'TypeSafe pasif — anahtar girmeden typesafe_decision aracı çalışmaz.',
      ts_status_off: 'TypeSafe anahtarı kayıtlı ama AYAR KAPALI — Jev tabanlı araçlar (browser_agent) çalışmaz.',
      ts_save: 'Kaydet',
      ts_test: 'Bağlantıyı Test Et',
      ts_testing: 'Test ediliyor…',
      ts_clear: 'Anahtarı Sil',
      ts_test_ok: 'TypeSafe bağlantısı başarılı — model: ',
      ts_test_fail: 'TypeSafe testi başarısız: ',
      ts_empty_toast: 'Anahtar boş — mevcut anahtar korundu',
    ...
    en: {
      ...
      tipAgentDm: 'Agent DM — agent-to-agent chat',
      ...
      p_custom_sub: 'Provide API URL + key, models are fetched automatically; add extra ones manually',
      p_custom_del: 'Delete',
      p_name_opt: 'Name (optional)',
      p_api_url: 'API URL',
      p_api_key: 'API Key',
      p_fetch: 'Fetch Models',
      p_models_manual: 'Extra models — add manually (one per line; added to fetched list)',
      p_save: 'Save',
      p_model_toast: 'Model: ',
      p_del_toast: 'Deleted: ',
      ...
      stt_seg_auto: 'Auto',
      stt_seg_whisper: 'Whisper (local)',
      tts_stt_title: 'Speech → Text',
      tts_stt_sub: 'Engine that transcribes your microphone',
      tts_tts_title: 'Text → Speech',
      tts_tts_sub: 'Voice reply engine and voice',
      stt_engine_auto: 'Auto — cloud if a key exists, otherwise local Whisper',
      stt_engine_local: 'Whisper — local/offline (large-v3-turbo)',
      stt_engine_groq: 'Groq Whisper — cloud (fast)',
      stt_engine_openai: 'OpenAI-compatible Whisper — cloud',
      tts_note: 'Edge TTS: free cloud engine — no key needed (Ahmet/Emel etc.). Piper: fully local/offline neural engine (downloads ~80MB on first use). OpenAI engine: compatible speech API (tts-1, tts-1-hd; voices alloy, echo, fable, onyx, nova, shimmer).',
      tts_engine: 'Engine',
      tts_edge_voice: 'Edge voice',
      tts_piper_voice: 'Piper voice',
      tts_piper_download: 'Download / prepare Piper',
      tts_piper_speed: 'Speaking speed',
      tts_piper_silence: 'Sentence pause',
      tts_piper_noise: 'Expressiveness',
      ...
      ws_h2: 'Web Search',
      ws_sub: 'Search chain: 1) SearXNG (local — auto-prioritized when running) 2) stealth (Chrome TLS impersonation) 3) built-in browser (direct Google) 4) TinyFish (if a key is set) 5) python multi-engine (ddgs / DDG+Bing+Mojeek). Order and on/off are editable below.',
      so_h2: 'Search Order',
      ...
      w_pane: 'WATCHERS',
      tipWatchLog: 'View watcher logs',
      w_log_title: 'Watcher Logs',
      w_log_clear: 'Clear selected watcher logs',
      w_log_cleared: 'Logs cleared',
      w_log_empty: 'No log entries for this watcher yet.',
      w_log_ev_ok: 'check',
      w_log_ev_trigger: 'trigger',
      w_log_ev_error: 'error',
      wa_vazgec: 'Cancel',
      wa_kaydet: 'save',
      wa_updated: 'Updated',
      wa_isim_ph: 'name',
      ...
      /* Cron edit */
      cr_update: 'Update',
      cr_cancel_edit: 'Cancel',
      cr_edit_hint: 'click: edit',
      cr_edit_loaded: 'Task loaded into form — edit and Update',
      cr_updated: 'Task updated',
      cr_added: 'Task added',
      cr_edit_cancelled: 'Edit cancelled',
      /* Watcher add form */
      w_add: '+',
      w_added: 'Watcher added',
      w_add_name: 'Name',
      w_add_url: 'https://...',
      w_add_path: 'json path (e.g. data.price)',
      w_add_value: 'threshold value',
      w_add_re: 'regex (optional)',
      w_kind_web: 'Web',
      w_kind_battery: 'Battery',
      w_kind_logs: 'Log',
      w_op_changed: 'changed',
      /* Agenda */
      aj_h2: 'Agenda',
      aj_sub: 'Upcoming cron jobs, reminders and watchers — nearest first.',
      aj_empty: 'Nothing scheduled.',
      aj_now: 'now',
      aj_later: 'from now',
      aj_ago: 'ago',
      aj_notify: 'Desktop notifications (toast) — on cron, reminder, watcher and agent finish',
      aj_notify_on: 'Notifications on',
      aj_notify_off: 'Notifications off',
      /* Email panel */
      mail_inbox: 'Inbox',
      mail_refresh: 'Refresh',
      mail_compose: 'New Mail',
      mail_to: 'to@example.com',
      mail_subject: 'Subject',
      mail_body: 'Message…',
      mail_send: 'Send',
      mail_sent: 'Mail sent',
      mail_send_fail: 'Could not send mail',
      mail_empty: 'Inbox is empty.',
      mail_reply: 'Reply',
      mail_read_fail: 'Could not read mail',
      mail_need_to: 'Recipient address required',
      us_reset_toast: 'Usage counters reset',
      ...
      tip_bb_restart: 'Restart — reopens the stuck page from scratch (session kept)',
      ...
      st_open: 'Open',
      st_del: 'Delete',
      sec_toggle: 'Ask permission: only for deletions (including Beast Code and Sandbox)',
      sec_sub: 'When off (default) NOTHING asks for permission anywhere — Beast Code, Sandbox and all tools are free; the agent has full authority. When on, approval is asked ONLY for deletions: an approval card in chat + a WhatsApp message. Other actions (write, edit, run commands, mail…) never ask.',
      ...
      sec_on_toast: 'Approval ON — the agent asks only for deletions',
      sec_off_toast: 'Approval OFF — nothing asks, everything free',
      sec_ok: 'Approve',
      sec_always: 'Always',
      sec_no: 'Deny',
      ...
      tipSandbox: 'Beast Sandbox — download a GitHub repo & work inside it',
      tipMcp: 'Beast MCP — external tool servers (Model Context Protocol)',
      ...
      store_likes: 'likes',
      store_builtin: 'Built-in',
      store_builtin_note: 'Ships with the agent',
      ...
      ts_status_set: 'TypeSafe active — saved key: *** · model: ',
      ts_status_unset: 'TypeSafe inactive — typesafe_decision does nothing without a key.',
      ts_status_off: 'TypeSafe key saved but the SWITCH IS OFF — Jev-powered tools (browser_agent) will not run.',
      ts_save: 'Save',
      ts_test: 'Test Connection',
      ts_testing: 'Testing…',
      ts_clear: 'Delete Key',
      ts_test_ok: 'TypeSafe connection OK — model: ',
      ts_test_fail: 'TypeSafe test failed: ',
      ts_empty_toast: 'Key is empty — existing key kept',
